...
技术地图生成后、规划方向前，用 `question` 询问已有知识；选择、单元掌握评估和课程结束评估也用 `question`，只问直接影响学习的信息。问题和选项必须参考技术地图中的方向、前置、边界和成本。每个单元讲解后先等待学习者阅读，再用与该单元目标直接相关的问题评估掌握程度；课程结束后评估整体掌握度，再选择开始新课或深化当前内容。工具不可用/失败即停止。用户问题直接回答，不能为每节课制造无关确认或继续确认。
